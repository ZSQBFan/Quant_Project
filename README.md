...
1.  **(ETL)**: 确保您的外部数据（如E/P, FCF）已被存入 `quant_data.db` 中的一个新表（例如 `stock_fundamentals`）。
2.  **`data/data_manager.py`**:
...
4.  **`factor_analysis/factors_complex.py`**:
...
1.  **`factor_analysis/rolling_weight_calculator.py`**:
